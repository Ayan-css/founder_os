...
Future<void> showClientDetailSheet(BuildContext context, {required Client client}) {
...
class _ClientDetailSheet extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final liveClient = ref.watch(clientsProvider).whenOrNull(
          data: (list) => list.firstWhereOrNull((c) => c.id == client.id)) ?? client;

    return DraggableScrollableSheet(
      initialChildSize: 0.68, minChildSize: 0.45, maxChildSize: 0.92, expand: false,
          ...
          Expanded(child: ListView(controller: scrollCtrl,
            ...
            children: [
              Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                _ClientAvatar(name: liveClient.name, status: liveClient.status),
                const SizedBox(width: 14),
                Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Text(liveClient.name, style: AppTypography.subheading, maxLines: 2, overflow: TextOverflow.ellipsis),
                  const SizedBox(height: 8),
                  Wrap(spacing: 6, runSpacing: 6, children: [
                    ClientStatusBadge(status: liveClient.status),
                    PaymentStateBadge(payment: liveClient.paymentState),
                  ]),
                ])),
                GestureDetector(
                  onTap: () => showClientFormSheet(ctx, client: liveClient),
              ...
              if (liveClient.deliverables.isNotEmpty) ...[
                Row(children: [
                  ...
                  Text('${liveClient.pendingDeliverableCount} pending', style: AppTypography.caption),
                ]),
                const SizedBox(height: 10),
                _DeliverableList(deliverables: liveClient.deliverables),
                const SizedBox(height: 20),
              ],
              if (liveClient.notes != null && liveClient.notes!.isNotEmpty) ...[
                ...
                Container(width: double.infinity, padding: const EdgeInsets.all(14),
                  ...
                  child: Text(liveClient.notes!, style: AppTypography.body.copyWith(height: 1.6))),
                const SizedBox(height: 20),
              ],
              const Divider(color: AppColors.borderSubtle, height: 1),
              ...
              Row(children: [
                ...
                Text(_dateFormat.format(liveClient.createdAt), style: AppTypography.caption.copyWith(color: AppColors.textSecondary)),
                const Spacer(),
                Text('Updated ', style: AppTypography.caption),
                Text(_dateFormat.format(liveClient.updatedAt), style: AppTypography.caption.copyWith(color: AppColors.textSecondary)),
              ]),
            ])),
        ])));
  }
...
class _ClientAvatar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(width: 52, height: 52,
      decoration: BoxDecoration(color: status.dimColor, shape: BoxShape.circle, border: Border.all(color: status.color.withOpacity(0.45), width: 1.5)),
      child: Center(child: Text(_initials, style: AppTypography.subheading.copyWith(color: status.color, fontSize: 18))));
  }
}
...
class _QuickPaymentRow extends ConsumerWidget {
  ...
  @override
  Widget build(BuildContext context, WidgetRef ref) {
    return Row(children: PaymentState.values.map((p) {
      ...
      return Expanded(child: GestureDetector(
        onTap: () {
          ...
          ref.read(clientsProvider.notifier).updateClient(client.copyWith(paymentState: p));
        },
        child: AnimatedContainer(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            ...
            const SizedBox(height: 4),
            Text(p.label, style: AppTypography.caption.copyWith(
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start,
        children: deliverables.map((d) {
          ...
          return Padding(padding: EdgeInsets.only(bottom: isLast ? 0 : 10),
            child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Container(width: 7, height: 7, margin: const EdgeInsets.only(top: 6, right: 10),
                decoration: BoxDecoration(color: AppColors.primary.withOpacity(0.6), shape: BoxShape.circle)),
              Expanded(child: Text(d, style: AppTypography.body.copyWith(height: 1.5))),
            ]));
        }).toList()));
  }
}
