    ...
    return Scaffold(
      ...
      body: SafeArea(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Padding(padding: const EdgeInsets.fromLTRB(16, 14, 16, 0),
          child: Row(children: [
            ...
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              async.whenOrNull(data: (list) => Text('${list.length} entries', style: AppTypography.bodySmall)) ?? const SizedBox.shrink(),
            ])),
          ])),
        ...
        Expanded(child: async.when(
          ...
          data: (list) => list.isEmpty ? _EmptyState() : _ReflectionList(reflections: list),
        )),
      ])),
    );
  }
...
class _ReflectionCardState extends State<_ReflectionCard> {
  ...
  String? _previewText() {
    ...
    if (r.distraction != null && r.distraction!.isNotEmpty) return r.distraction;
    if (r.forward != null && r.forward!.isNotEmpty) return r.forward;
    if (r.improvement != null && r.improvement!.isNotEmpty) return r.improvement;
    return null;
  }

  String? _answerAt(int i) => switch (i) { 0 => widget.reflection.distraction, 1 => widget.reflection.forward, 2 => widget.reflection.improvement, _ => null };
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          color: _isToday ? AppColors.primaryDim.withOpacity(0.35) : AppColors.surface,
          borderRadius: BorderRadius.circular(AppConstants.radiusMD),
          border: Border.all(color: _isToday ? AppColors.primary.withOpacity(0.28) : AppColors.border)),
        child: Column(children: [
          Padding(padding: const EdgeInsets.all(14),
            child: Row(children: [
              Container(width: 36, height: 36,
                decoration: BoxDecoration(
                  color: count == 3 ? AppColors.successDim : count == 2 ? AppColors.primaryDim : AppColors.surfaceHighlight,
                  borderRadius: BorderRadius.circular(9), border: Border.all(color: AppColors.border)),
                child: Center(child: Text('$count/3', style: AppTypography.caption.copyWith(
                  fontWeight: FontWeight.w700, color: count == 3 ? AppColors.success : AppColors.textSecondary)))),
              const SizedBox(width: 12),
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Row(children: [
                  Flexible(child: Text(_dateFormat.format(AppDateUtils.fromDbDate(r.date)), style: AppTypography.bodySmall.copyWith(color: AppColors.textSecondary))),
                  if (_isToday) ...[const SizedBox(width: 6), _TodayBadge()],
                ]),
                if (!_expanded) ...[const SizedBox(height: 3), Text(_previewText() ?? 'Tap to view', style: AppTypography.caption, maxLines: 1, overflow: TextOverflow.ellipsis)],
              ])),
              AnimatedRotation(turns: _expanded ? 0.5 : 0.0, duration: AppConstants.animNormal,
                child: const Icon(Icons.keyboard_arrow_down_rounded, color: AppColors.textMuted, size: 20)),
            ])),
          AnimatedSize(
            ...
            child: _expanded
                ? Padding(padding: const EdgeInsets.fromLTRB(14, 0, 14, 14),
                    child: Column(children: [
                      ...
                      ...List.generate(3, (i) => _ExpandedAnswer(prompt: reflectionPrompts[i], answer: _answerAt(i))),
                    ]))
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(padding: const EdgeInsets.only(bottom: 14),
      child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(prompt.question, style: AppTypography.caption.copyWith(color: AppColors.textMuted, fontWeight: FontWeight.w600)),
          const SizedBox(height: 5),
          Text(hasAnswer ? answer! : '— left blank',
            style: hasAnswer ? AppTypography.body.copyWith(height: 1.58)
                : AppTypography.caption.copyWith(color: AppColors.textDisabled, fontStyle: FontStyle.italic)),
        ])),
      ]));
  }
}
...
class _TodayBadge extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    decoration: BoxDecoration(color: AppColors.primaryDim, borderRadius: BorderRadius.circular(4)),
    child: Text('today', style: AppTypography.caption.copyWith(color: AppColors.primaryLight, fontSize: 9)));
}
...
class _EmptyState extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
    ...
    Text('Your nightly reflections\nwill appear here.', style: AppTypography.body, textAlign: TextAlign.center),
  ]));
}