            ...
            if (allDone) ...[
              ...
              SliverPadding(
                  padding: const EdgeInsets.symmetric(
                      horizontal: AppConstants.spaceMD),
                  sliver:
                      const SliverToBoxAdapter(child: _CelebrationBanner())),
            ],
            const SliverToBoxAdapter(
    ...
    return Row(children: [
      Expanded(
          child: _MiniCard(
              ...
              title: 'Captures',
              subtitle:
                  captureCount == 0 ? 'None today' : '$captureCount today',
              onTap: () => Navigator.push(context,
                  MaterialPageRoute(builder: (_) => const CapturesScreen())))),
      const SizedBox(width: 8),
      Expanded(
          child: _MiniCard(
              icon: '🎬',
              iconBg: AppColors.surfaceHighlight,
              title: 'Pipeline',
              ...
              onTap: () => Navigator.push(
                  context,
                  MaterialPageRoute(
                      builder: (_) => const ContentPipelineScreen())))),
    ]);
  }
}
...
class _BottomUtilityRow extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final overdue =
        summaryAsync.whenOrNull(data: (m) => (m['overdue'] ?? 0)) ?? 0;
    final active =
        summaryAsync.whenOrNull(data: (m) => (m['active'] ?? 0)) ?? 0;
    final clientSubtitle = overdue > 0
        ? '$overdue overdue 🔴'
        : active == 0
            ? 'No active clients'
            : '$active active';
    ...
    final reflectionDone =
        reflectionAsync.whenOrNull(data: (r) => r != null && !r.isEmpty) ??
            false;
    final isEvening = DateTime.now().hour >= 17;
    final reflectionSubtitle = reflectionDone
        ? 'Done today ✅'
        : isEvening
            ? 'Time to reflect 🌙'
            : 'Reflect tonight';
    ...
    return Row(children: [
      Expanded(
          child: _MiniCard(
              ...
              iconBorderColor:
                  overdue > 0 ? AppColors.error.withOpacity(0.35) : null,
              title: 'Clients',
              ...
              onTap: () => Navigator.push(
                  context,
                  MaterialPageRoute(
                      builder: (_) => const ClientTrackerScreen())))),
      const SizedBox(width: 8),
      Expanded(
          child: _MiniCard(
              icon: reflectionDone
        ...
        child: Row(children: [
          ...
          Expanded(
              child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                Text('All priorities done!',
                    style: AppTypography.bodyLarge
                        .copyWith(color: AppColors.success)),
                Text("You're executing at a high level today.",
