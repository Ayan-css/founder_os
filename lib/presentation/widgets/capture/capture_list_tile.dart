...
class CaptureListTile extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    return Dismissible(
      ...
      onDismissed: (_) { HapticFeedback.mediumImpact(); ref.read(capturesProvider.notifier).delete(capture); },
      child: Container(
        ...
        child: Padding(
          ...
          child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Container(
              ...
              child: Center(child: Text(capture.type.emoji, style: const TextStyle(fontSize: 16))),
            ),
            const SizedBox(width: 12),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(capture.content, style: AppTypography.bodyLarge.copyWith(height: 1.5), maxLines: 6, overflow: TextOverflow.ellipsis),
              ...
              Row(children: [
                ...
                const SizedBox(width: 8),
                Text(_timeFormat.format(capture.createdAt), style: AppTypography.caption),
              ]),
            ])),
          ]),
        ),
...
class _TypeLabel extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(color: AppColors.primaryDim, borderRadius: BorderRadius.circular(5)),
      child: Text(type.label, style: AppTypography.caption.copyWith(color: AppColors.primaryLight, fontSize: 10)),
    );
  }
}
