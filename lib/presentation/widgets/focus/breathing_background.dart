...
class BreathingBackground extends StatefulWidget {
  const BreathingBackground({super.key, required this.child, required this.isActive});
  final Widget child;
  final bool isActive;
  @override
...
class _BreathingBackgroundState extends State<BreathingBackground> with SingleTickerProviderStateMixin {
  ...
  void didUpdateWidget(BreathingBackground old) {
    ...
    if (widget.isActive && !old.isActive) {
      ...
      _ctrl.animateTo(0.0, duration: const Duration(seconds: 2), curve: Curves.easeOut);
    }
  }
  ...
  Widget build(BuildContext context) {
    return AnimatedBuilder(
      ...
      builder: (_, child) => Container(
        ...
        child: CustomPaint(painter: _BreathPainter(value: _anim.value), child: child),
      ),
      child: widget.child,
    );
  }
...
class _BreathPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawCircle(center, maxR * (0.45 + value * 0.35), Paint()
      ..shader = RadialGradient(colors: [AppColors.primary.withOpacity(0.09 + value * 0.06), Colors.transparent])
          .createShader(Rect.fromCircle(center: center, radius: maxR)));
    canvas.drawCircle(center, maxR * (0.75 + value * 0.25), Paint()
      ..shader = RadialGradient(colors: [AppColors.primaryDim.withOpacity(0.06 + value * 0.03), Colors.transparent])
