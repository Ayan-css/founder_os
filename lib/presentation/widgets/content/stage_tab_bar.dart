...
class StagePillTabBar extends StatelessWidget {
  const StagePillTabBar({super.key, required this.currentStage, required this.counts, required this.onStageTap});
        ...
        itemBuilder: (_, i) {
          ...
          return Padding(padding: const EdgeInsets.only(right: 8),
            child: GestureDetector(
              onTap: () { HapticFeedback.selectionClick(); onStageTap(stage); },
              child: AnimatedContainer(
                duration: AppConstants.animNormal, curve: Curves.easeOut,
                padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 9),
                decoration: BoxDecoration(
                  color: isSelected ? stage.dimColor : AppColors.surfaceElevated,
                  borderRadius: BorderRadius.circular(50),
                  border: Border.all(color: isSelected ? stage.color.withOpacity(0.5) : AppColors.border, width: isSelected ? 1.5 : 1),
                  boxShadow: isSelected ? [BoxShadow(color: stage.glowColor, blurRadius: 10, spreadRadius: -2)] : []),
                child: Row(mainAxisSize: MainAxisSize.min, children: [
                  Text(stage.emoji, style: const TextStyle(fontSize: 13)),
                  const SizedBox(width: 6),
                  Text(stage.label, style: AppTypography.bodySmall.copyWith(
                  ...
                  if (count > 0) ...[
                    ...
                    AnimatedContainer(
                      ...
                      decoration: BoxDecoration(
                        color: isSelected ? stage.color.withOpacity(0.22) : AppColors.border,
