      ...
      child: GestureDetector(
        ...
        child: Container(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Container(height: 3, decoration: BoxDecoration(gradient: LinearGradient(colors: [stage.color.withOpacity(0.85), stage.color.withOpacity(0.15)]))),
            Padding(padding: const EdgeInsets.fromLTRB(14, 12, 12, 12),
              child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Text(item.title, style: AppTypography.bodyLarge.copyWith(height: 1.4), maxLines: 3, overflow: TextOverflow.ellipsis),
                  if (item.notes != null && item.notes!.isNotEmpty) ...[
                    const SizedBox(height: 5),
                    Text(item.notes!, style: AppTypography.bodySmall.copyWith(height: 1.45), maxLines: 2, overflow: TextOverflow.ellipsis),
                  ],
                  const SizedBox(height: 10),
                  Row(children: [
                    _StagePill(stage: stage),
                    const SizedBox(width: 8),
                    Text('· ${_dateFormat.format(item.createdAt)}', style: AppTypography.caption),
                  ]),
                ])),
                if (hasNext) ...[const SizedBox(width: 10), _AdvanceButton(item: item)],
              ])),
          ])),
      ));
  }
...
class _StagePill extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(color: stage.dimColor, borderRadius: BorderRadius.circular(5)),
      child: Row(mainAxisSize: MainAxisSize.min, children: [
        Text(stage.emoji, style: const TextStyle(fontSize: 9)),
        const SizedBox(width: 4),
        Text(stage.label, style: AppTypography.caption.copyWith(color: stage.color, fontSize: 10, fontWeight: FontWeight.w600)),
      ]));
  }
}
...
class _AdvanceButton extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return GestureDetector(
      onTap: () { HapticFeedback.lightImpact(); ref.read(contentProvider.notifier).moveToNextStage(item); },
      child: Tooltip(message: 'Move to ${next.label}',
        child: Container(width: 42, height: 42,
          decoration: BoxDecoration(color: next.dimColor, borderRadius: BorderRadius.circular(10), border: Border.all(color: next.color.withOpacity(0.3))),
          child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
            Text(next.emoji, style: const TextStyle(fontSize: 13)),
            const SizedBox(height: 1),
            Icon(Icons.arrow_forward_rounded, size: 11, color: next.color),
          ]))));
  }
}
