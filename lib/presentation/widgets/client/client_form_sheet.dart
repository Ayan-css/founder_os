...
Future<void> showClientFormSheet(BuildContext context, {Client? client}) {
  return showModalBottomSheet(
      ...
      builder: (_) => _ClientFormSheet(client: client));
}
...
class _ClientFormSheetState extends ConsumerState<_ClientFormSheet> {
  ...
  void initState() {
    ...
    _deliverablesCtrls = existing.isEmpty
        ? [TextEditingController()]
        : existing.map((d) => TextEditingController(text: d)).toList();
  ...
  void dispose() {
    _nameCtrl.dispose();
    _notesCtrl.dispose();
    for (final c in _deliverablesCtrls) c.dispose();
    super.dispose();
  }

  void _addDeliverableRow() =>
      setState(() => _deliverablesCtrls.add(TextEditingController()));

  void _removeDeliverableRow(int i) {
    if (_deliverablesCtrls.length <= 1) {
      _deliverablesCtrls.first.clear();
      setState(() {});
      return;
    }
  ...
  List<String> get _deliverables => _deliverablesCtrls
      .map((c) => c.text.trim())
  ...
  Future<void> _save() async {
    ...
    if (_isEdit) {
      await notifier.updateClient(widget.client!.copyWith(
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              ...List.generate(
                  _deliverablesCtrls.length,
                  (i) => DeliverableRow(
                      key: ValueKey(i),
                      controller: _deliverablesCtrls[i],
                      index: i,
                      autofocus: false,
                      onRemove: () => _removeDeliverableRow(i),
                      onSubmitted: _addDeliverableRow)),
              const SizedBox(height: 20),
              ...
              TextField(
                  ...
                  decoration: const InputDecoration(
                      hintText: 'Anything else to remember... (optional)')),
              const SizedBox(height: 22),
              SizedBox(
                width: double.infinity,
                child: ElevatedButton(
                  onPressed: (_saving || !_hasName) ? null : _save,
                  child: _saving
...
class _StatusSelector extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Column(
        children: ClientStatus.values.map((s) {
